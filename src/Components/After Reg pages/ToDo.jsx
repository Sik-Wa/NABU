...
export const ToDo = () => {
  const [todos, setTodos] = useState([
    ...
    {
      id: 3,
      text: "Have youtube english",
      checked: true,
      time: "12 hrs",
    },
    {
      id: 4,
      text: "Get done with pastpapers ",
      checked: false,
      time: "1 hrs",
    },
  ]);
...
const Header = () => {
  return (
    <div className="mb-6">
      <h1 className="text-xl font-medium text-white flex items-center justify-center">Good morning! ☀️</h1>
  ...
  return (
    <div className="fixed bottom-6 left-1/2 w-full max-w-xl -translate-x-1/2 px-4">
        ...
        {visible && (
          ...
          >
            <textarea
              ...
              className="h-24 w-full resize-none rounded bg-zinc-900 p-3 text-sm
               text-zinc-50 placeholder-zinc-500 caret-zinc-50 focus:outline-0"
            />
            <div className="flex items-center justify-between">
              ...
              <button
                type="submit"
                className="rounded bg-indigo-600 px-1.5 py-1 text-xs text-indigo-50
      ...
      <button
        ...
        className="grid w-full place-content-center rounded-full border
         border-zinc-700 bg-zinc-900 py-3 text-lg text-white transition-colors
...
const Todo = ({ removeElement, handleCheck, id, children, checked, time }) => {
  ...
  return (
    <motion.div
      ref={scope}
      layout
      className="relative flex w-full items-center gap-3 rounded
       border border-zinc-700 bg-zinc-900 p-3"
    >
      <input
        type="checkbox"
        checked={checked}
      ...
      <div className="ml-auto flex gap-1.5">
        <div className="flex items-center gap-1.5 whitespace-nowrap rounded
         bg-zinc-800 px-1.5 py-1 text-xs text-zinc-400">
          <FiClock />
          <span>{time}</span>
        </div>
        <button
          onClick={() => removeElement(id)}
          className="rounded bg-red-300/20 px-1.5 py-1 text-xs text-red-300
           transition-colors hover:bg-red-600 hover:text-red-200"
        >
          <FiTrash2 />
        </button>
      </div>
